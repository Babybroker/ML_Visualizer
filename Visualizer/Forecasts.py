...
class ForecastVisualizer(VisualizerBody):
    ...
    def plot_scatter(self, extra_title_addition=None,  custom_xlabel=None, custom_ylabel=None, hue=None,
        ...
        if remove_outliers:
            resultdf['z_score'] = (resultdf[self.target_col] - resultdf[self.target_col].mean())/resultdf[self.target_col].std(ddof=0)
        ...
        f, ax = plt.subplots(figsize=(8, 8))
        f = sns.scatterplot(data=resultdf, x=self.target_col, y=self.predict_col, s=2, alpha=0.5, hue=hue, palette=custom_palette)
